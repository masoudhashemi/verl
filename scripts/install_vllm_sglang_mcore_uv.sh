...
echo "1. install inference frameworks and pytorch they need (using uv)"
...
echo "2. install basic packages (using uv)"
...
echo "3. install FlashAttention and FlashInfer (using uv)"
...
if [ $USE_MEGATRON -eq 1 ]; then
    echo "4. install TransformerEngine and Megatron (using uv)"
...
echo "5. May need to fix opencv (using uv)"
...
if [ $USE_MEGATRON -eq 1 ]; then
    echo "6. Install cudnn python package (avoid being overridden) (using uv)"
    uv pip install nvidia-cudnn-cu12==9.8.0.87
fi

echo "Successfully installed all packages (uv)"
